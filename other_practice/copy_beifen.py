...
for i in listpath:
    if not os.path.isdir(i):
        ...
        if filetype=='webp':
            ...
            os.rename(os.path.join(path,i),os.path.join(path,"linxu_other" + str(other_number)+'.'+filetype))
            other_number=other_number+1
            os.chdir(path)
            if not os.path.exists(filetype):
                os.mkdir(filetype)
                print(f"{filetype}目录创建成功")
print(f"文件夹下图片的个数为{jpg_num}")
print(f"文件夹下其他文件的个数为{other_number}")
